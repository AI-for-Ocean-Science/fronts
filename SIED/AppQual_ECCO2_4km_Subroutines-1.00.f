...
c
c     subroutine ECCO2_4km_GetDateTime(ImageName, Year, Month, 
c     1     YearT, MonthT, Day, Hour, Minute, Second)
c     
c     subroutine ECCO2_4km_GetGeoFilename(ImageName, YearC, MonthC, 
c     1     GeoNameFull)
c     
c     subroutine ECCO2_4km_GetSSTOutData( FileNameIn, ConditionedSST, 
c     1     SecondsSince1970)
c     
c     subroutine ECCO2_4km_CreateOutputSSTFile( FileNameOut, ncOutID, 
c     1     sstIDout, ClearIDout, FrontsIDout, DayOrNightIDout,
c     2     SecondsSinceID, Start, YearC, MonthC, DayC)
c     
c      subroutine ECCO2_4km_GetGeoData( FileNameIn, LatIn, LonIn, 
...
c       AVHRR data
c     04/22/15 - JPS - Modified from the AVHRR_hrpt file to run on
c       ECCO2 llc_2160 data
c     
c***********************************************************************
      subroutine ECCO2_4km_GetDateTime( ImageName, Year, Month, 
...
c---------------------------------start here --------------------------- 
      ...
      if(Debug .eq. 1) print *, 'GetTimes #100 : ImageName is :' //
     1     ImageName // '::'
...
c     Get year, month, day. 
      ...
      read( ImageName(5:8), '(i4)') YearT
      read( ImageName(9:10), '(i2)') MonthT
      read( ImageName(11:12), '(i2)') Day
...
c     and do all that, so skip for now.
      ...
      end subroutine ECCO2_4km_GetDateTime

c***********************************************************************
      subroutine ECCO2_4km_GetGeoFilename(ImageName, YearC, 
...
c     the SST file. GeoNameFull is used to create the geospatial file.
      ...
      GeoName = 'GeoLoc/ecco2_llc_2160_geoloc.nc'
     ...
     1     'GeoName::', trim(GeoName), '::'
      ...
      end subroutine ECCO2_4km_GetGeoFilename

c***********************************************************************
      subroutine ECCO2_4km_GetSSTOutData( FileNameIn, sstOut, 
     1     SecondsSince1970)
c***********************************************************************
c     
...
c     
      ...
      if(Debug .eq. 1) print *, 'ECCO2_GetSSTData #000: ',
...
c     Allocate input and output variables.
     ...
     *     stat=ierr)
      ...
      if (ierr .ne. 0) then
         write(UnitLog,*) 'ECCO2_GetSSTData #100: Allocation error, ',
     1        'exiting.'
         stop 'ECCO2_GetSSTData #100: Allocation error, exiting.'
      endif
...
c     not, the time at each pixel location. 
      ...
      if(Debug .eq. 1) print *, 'ECCO2_GetSSTData #001: ',
     1     'Got Variable: Time'

c     Is the time read within 24 hours of the time calculated earlier?

      SecondsSinceTemp = (SecondsIn-719529)*86400

      if(abs(SecondsSinceTemp-SecondsSince1970) .gt. 86400*3/2) then
         write(UnitLog, *) 'ECCO2_GetSSTData #110: There is a problem',
     1        'with the time read in: ', SecondsSinceTemp, 
     2        'It is not close to the value determined from the file ',
     3        'name: ', SecondsSince1970
         print *, 'ECCO2_GetSSTData #110: There is a problem',
     1        'with the time read in: ', SecondsSinceTemp, 
     2        'It is not close to the value determined from the file ',
     3        'name: ', SecondsSince1970
         stop
...
c     Get the input variable IDs
     ...
     1     sstIDin)
      ...
      if(Debug .eq. 1) print *, 'ECCO2_GetSSTData #002: ',
     1     'Got Variable: SST'
...
c     are consistent with xLenIn and yLenIn
      ...
      if(debug .eq. 1) then
         write(UnitLog,*) 'ECCO2_GetSSTData #120: nxN, nyN: ', nxN, nyN
         print *, 'ECCO2_GetSSTData #120: nxN, nyN: ', nxN, nyN
      endif

      if( nxN .gt. LenXIn) then
         write(UnitLog,*) 'WriteSSTData #130 ERROR ** Long orbit: ', nxN
         print *, 'ECCO2_GetSSTData #130: ERROR ** Long orbit: ', nxN
         stop '******* Dimension error *******'
      endif

      if( nyN .gt. LenYIn) then
         write(UnitLog,*) 'ECCO2_GetSSTData #140: ERROR *** Bad # of ',
     1        'elements: ', nyN, ' should be less than', LenYIn
         print *, 'ECCO2_GetSSTData #140: Error *** Bad # of ',
     1        'elements: ', nyN, ' should be less than', LenYIn
         stop '******* Dimension error *******'
      endif
...
c     Get the missing values for the input SST, Lat and Lon
     ...
     1     RsstFillValue)
      ...
      if(status .ne. nf90_noerr) call handle_err(status)
c      RsstFillValue = -999
      if(debug .eq. 1) print *, 'ECCO2_GetSSTData #150: SST input  ',
...
c      correspond to that in the input file.
     ...
     1     SSTScaleFactorIn)
      ...
      if (((DegreesK .eq. 0) .and. (abs(InputOffset - sstOffsetIn -
     1        273.15) .gt. 2)) .or. ((DegreesK .eq. 1) .and. 
     2     (abs(InputOffset - sstOffsetIn) .gt. 2))) then
         print *, 'ECCO2_GetSSTData #155: SST offset from ',
     ...
     3        'file: ', sstOffsetIn, ' or ', SSTScaleFactorIn
            stop 'ECCO2_GetSSTData #156: STOPPING.'
      endif
...
c     Now allocate variable for input SST.
      ...
      allocate(TempSST(nxN,nyN,1),stat=ierr)
      if (ierr .ne. 0) then
         write(UnitLog,*) 'ECCO2_GetSSTData #160: Allocation error.'
         stop 'ECCO2_GetSSTData #160: Allocation error. STOPPING.'
      endif
...
c     bad), SST will also be set to the fill value
      ...
      do 1043 jy=1,nyN
         do 1044 ix=1,nxN
            if(TempSST(ix,jy,1) .ne. RsstFillValue) then 
...
c     Now clean up.
      ...
      end subroutine ECCO2_4km_GetSSTOutData

c**********************************************************************
      subroutine ECCO2_4km_CreateOutputSSTFile( FileNameOut, 
...
c     AMSR-E or TMI
      ...
      Loc1 = index( SatName, 'ecco2_4km')
      
      Title = BlankFill(:len(Title))
      Title  = 'Conditioned ' // trim(SatName_uc) // ' '
     1     // trim(ParameterName) // ' fields from the NASA Jet '
     2     // 'Propulsion Lab.'
...
c     middle part.
     ...
     6     'developed by and obtained from the ECCO2 ' //
     7     ' Model SST dataset}  See the license ' //
     8     'attribute for information regarding use and ' //
     9     're-distribution of these data.'

      license = BlankFill(:len(license))
      license = 'The fields associated with the ECCO2 data were '
     1     // 'made available from the California Institute of '
     2     // ' Technology, Jet Propulsion Lab courtesy of '
     3     // ' Dimitris Menemenlis.'
     
      contributor_name = BlankFill(:len(contributor_name))
      contributor_name = 'California Institude of Technology: ' //
     1     ' Jet Propulsion Lab. http://ecco2.jpl.nasa.gov'

      contributor_role = BlankFill(:len(contributor_role))
      if(Loc1 .eq. 1) then
         contributor_role = 'Generated SST fields from the ECCO2 ' //
     1        ' llc_2160 model run.'
      else
c         contributor_role = 'Generated SST fields from the TMI ' //
c     1        'data stream.'
      endif
     ...
     1     'Graduate School of Oceanography'
      ...
      ProgVersionNo = AppQual_ECCO2_4km_VersionNo
     ...
     5     publisher_url, publisher_institution, cdm_data_type)
      ...
      end subroutine ECCO2_4km_CreateOutputSSTFile

c***********************************************************************
      subroutine ECCO2_4km_GetGeoData( FileNameIn, LatIn, LonIn, 
     1     SecondsSince1970, FileNameOut, NumClear, YearC, MonthC, Day, 
     2     Hour, Minute, Second, GeoNameFull)
c***********************************************************************
c     
c     This subroutine will get the input SST data for ECCO2. 
c     
c     Will also divide the input by 10 as agreed with Pierre for CMS
c     SSTs in centidegrees.
c     
...
c     CountECCO2- the number of input elements to read.
      integer CountECCO2(3)
...
c     sent us.
      ...
      integer Day, Hour, ncid_geo, varid_lat, varid_lon
...
c     
      ...
      if(Debug .eq. 1) print *, 'ECCO2_GetGeoData #000: ',
     1     'trim(FileNameIn)::', trim(FileNameIn), '::'
...
c     Need to read a separate lat,lon file. Generate the filename.
         ...
         if(Debug .eq. 1) print *, 'ECCO2_GetGeoData #120: ', 
     1        'FileNameIn::', trim(FileNameIn), '::'
         LatLonFileIn = blankfill(:len(LatLonFileIn))
         LatLonFileIn = trim(BaseDir) // 'GeoLoc/' //
     1        'ecco2_llc_2160_geoloc.nc'

         if(Debug .eq. 1) print *, 'ECCO2_GetGeoData #130, Loc1: ',
     2        Loc1, ' LatLonFileIn::', trim(LatLonFileIn), '::'

c     Now open and read the file.

         status = nf90_open(LatLonFileIn,NF90_NOWRITE,ncid_geo)
         if(status .ne.  nf90_noerr) call handle_err(status)
         
         status = nf90_inq_varid(ncid_geo,'lon',varid_lon)
         if(status .ne.  nf90_noerr) call handle_err(status)
         status = nf90_inq_varid(ncid_geo,'lat',varid_lat)
         if(status .ne.  nf90_noerr) call handle_err(status)

         if(Debug .eq. 1) print *, 'ECCO2_GetGeoData #140, got ',
     1        'Lon/Lat varid::'
         
         status = nf90_get_var(ncid_geo,varid_lon,Lonin)
         if(status .ne.  nf90_noerr) call handle_err(status)
         status = nf90_get_var(ncid_geo,varid_lat,Latin)
         if(status .ne.  nf90_noerr) call handle_err(status)
         
         if(Debug .eq. 1) print *, 'ECCO2_GetGeoData #150: ',
     1        'Read LatLon::'
...
c     close the input file.
      ...
      end subroutine ECCO2_4km_GetGeoData
